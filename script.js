...
/**
 * Updates all balance displays across the app
 */
function updateBalanceUI() {
    const balDisplay = document.getElementById('balance-display'); // Top Nav
    const walletBal = document.getElementById('wallet-balance'); // Wallet Page

    if (balDisplay) balDisplay.innerHTML = `${balance.toFixed(2)} <small>KES</small>`;
    if (walletBal) walletBal.innerText = `${balance.toFixed(2)} KES`;

    // Also update the hidden data attribute if you're using one
...
function updatePrediction() {
    // Generate a 'suggested' exit point for the user
    // This could be based on the average of the last 5 rounds
    const history = getPastMultipliers(); // e.g., [1.2, 4.5, 1.1, 2.0]
    const avg = history.reduce((a, b) => a + b, 0) / history.length;

    currentPrediction = (avg * 0.8).toFixed(2); // Suggest a safe exit
    document.getElementById('prediction-display').innerText = `${currentPrediction}x`;
}
